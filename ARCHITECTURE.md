# EntreprenAIs Factory — 公式ホームページ＋公式エージェント自動生成システム

> **EntreprenAIs Factory** は、店舗・地域などの **公開情報** から、多言語対応の **公式ホームページ**（静的サイト）と、そのページ上で来訪者を案内する **公式エージェント（AI アンバサダー）** を大量に自動生成して公開するシステムです。
> このリポジトリ（`taru0216/factory-entreprenais-com`）はそのまま **fork / clone して利用** できます。
> 本ドキュメントは外部パートナーのエンジニア向けに、まず **「生成済みのホームページをどう使うか（ホスティング）」** を実践的に説明し、続いて全体像（仕組み・アーキテクチャ。公式エージェントの設計を含む）を高レベルで説明します。
...
**EntreprenAIs Factory** は、公開情報をクロール・正規化し、そこから対象（店舗・自治体など）ごとに次の **2 本柱** を **大量に自動生成** するシステムです。

1. **公式ホームページ**: 多言語対応の自己完結した静的サイト。
2. **公式エージェント（AI アンバサダー）**: その公式ホームページ上で来訪者を案内する AI エージェント。ページに JavaScript として組み込まれ、EntreprenAIs のエージェントシステムをページ上にオーバーレイします（設計詳細は「8. 公式エージェント（AI アンバサダー）」参照）。

生成対象は **カテゴリ** ごとに分かれており、本ドキュメント時点では次の 2 カテゴリがあります。

| カテゴリ識別子 | 対象 | 公開パス | 公式エージェント |
|--------------|------|---------|----------------|
| **`foodre`** | 飲食店 | `/foodre/{retty_id}/` | 今後搭載予定（現状は未実装） |
| **`cities`** | 自治体（地域情報） | `/cities/{code}/` | **稼働中**（例: 海士町 /cities/32525/） |
...
- 公式ホームページの生成物は **完全に自己完結した静的 HTML**（外部 CSS フレームワーク・JS フレームワーク非依存）。そのままどんな配信基盤にも載せられます。
- 情報は **1 ファイル 1 対象の `store.json`**（自治体は `municipality.json`）に集約（Single Source of Truth）。HTML も公式エージェントの知識ベースも、この JSON を元にします。
- **多言語対応**（日本語に加え en / zh / ko / th / vi / zh-TW のフィールドを持つ）。
- **公式エージェント**は静的 HTML に JavaScript を組み込んでオーバーレイ表示するため、静的サイトの「ファイルを置くだけ」という手軽さを保ったまま AI 案内を載せられます。
...
- **公開中の例**:
  ...
  - 自治体カテゴリの例（**公式エージェント稼働中**）: https://factory.entreprenais.com/cities/32525/ — 海士町。ページ上で AI アンバサダーが来訪者を案内します。
- **目標とする基本デザイン**: https://entreprenais.com/sites/banwaen-ikebukuro/ — 現在の生成テンプレートは暫定品質であり、このサイトの完成度を基準に改良していきます（詳細は「10.1 デザイン方針」参照）。
...
### 2.5 ホスティング例: Fastly（CDN として配信する）

Fastly を使う場合の手順です。

> **重要な前提**: Fastly は **CDN / エッジ配信プラットフォーム** であり、S3 のように「ファイルを直接アップロードして公開する」仕組みではありません。**静的ファイルの置き場（オリジン）を別途用意し、その前段に Fastly を CDN として置く** のが基本構成です。Fastly はオリジンの内容をキャッシュ・配信する役割を担います。

```
[利用者] → [Fastly エッジ（CDN・キャッシュ・TLS）] → [オリジン（S3 / GCS / 任意の Web サーバ）]
```

#### 手順

1. **オリジンに静的ファイルを配置する**
   生成済みの `foodre/` / `cities/` 等の静的ファイルを、オリジンとなるストレージ／サーバに置きます。オリジンには S3 / GCS バケットの静的ホスティング、または nginx/Apache 等の任意の Web サーバが使えます（オリジン構築は「2.3 ホスティング例: Amazon S3」の手順をそのまま流用できます）。

2. **Fastly サービスを作成し、オリジンを backend に設定する**
   Fastly で新規サービスを作成し、手順 1 のオリジンを backend（origin host）として登録します。

3. **配信ドメインと TLS を構成する**
   サービスに配信ドメイン（独自ドメイン等）を設定し、TLS 証明書を構成します。これにより HTTPS 配信が有効になります。

4. **キャッシュを設定する**
   生成物は完全に静的な HTML/アセットなので **キャッシュ可能** です。`Cache-Control` ヘッダや Fastly 側の TTL を適宜設定します。静的サイトなので積極的にキャッシュして問題ありません（コンテンツ更新時はパージで反映）。

5. **（必要に応じて）VCL でリライトを設定する**
   サブパス配信などパス変換が必要な場合は、VCL でリクエストパスのリライト等を設定します。ページ内リンクは `/foodre/...` 起点の絶対パスのため、別サブパスに載せる場合はここで吸収します（パスの扱いは「2.6 自社サイトへの組み込み」の注記も参照）。

> **補足**:
> - より簡易には、Fastly のオリジンに **GCS / S3 の静的ホスティングをそのまま使う** のが手軽です（オリジン構築・公開は既存の静的ホスティング手順を流用し、その前段に Fastly を置くだけ）。
> - 高度な用途では **Fastly Compute@Edge** や **KV ストア** をオリジン／エッジロジックとして使う選択肢もありますが、静的サイトの配信には上記のオリジン + CDN 構成で十分です。
...
>
> このほか、`store.json` スキーマだけを流用する・クロール&ビルド機構ごと再利用する、といった組み込み方も可能です。詳しくは「9. fork / clone / 組み込み手順」を参照してください。

### 2.7 ビルドフレームワークの互換性に関する注意（重要）

> **推奨: 外部での利用・自社サイトへの組み込みには、ビルド済みの静的ファイル（生成済み HTML）を再利用してください。**

EntreprenAIs Factory の **ビルドフレームワーク**（クローラ `scripts/crawl_retty.py`・ページテンプレート・Astro/ビルド機構 `site-builder/`・`store.json` スキーマ・GitHub Actions ワークフロー等）は、**継続的に改良を加えています**。そのため、**バージョン間で互換性は維持されません**（予告なく破壊的変更が入りうる前提です）。

- **推奨（安定）**: 「2. ホスティング方法」で説明した **生成済みの静的ファイル（ビルド済み HTML）をそのまま再利用** してください。生成物は自己完結した静的 HTML であり、ビルド機構のバージョンに依存せず安定して使えます。
- **非推奨（変更リスクあり）**: **ビルド機構そのもの**（クローラ・テンプレート・Astro 設定・ワークフロー）や **`store.json` スキーマ** に依存した組み込みは、将来の改良で **動かなくなるリスク** があります。
- これらのビルド機構・スキーマに依存する組み込み方（「9.2 パターン B / パターン C」）を採る場合は、互換性が維持されないことを前提に、利用するバージョンを固定する・変更に追従する運用を各自でご用意ください。

> 要するに、**安定して使うなら「生成済みの静的ファイルを再利用する」のが確実** です。ビルド機構や `store.json` スキーマに依存した組み込みは、フレームワーク改良に伴う破壊的変更の影響を受けることを織り込んでください。
...
クロールは「**身元を明示し、負荷に配慮し、公開情報のみを取得する**」運用を徹底しています。

- **User-Agent**: 全リクエストで身元を明示します。
  ```
  EntreprenAIs-Factory-crawler/0.1 (+https://entreprenais.com/#contact)
  ```
  UA に問い合わせ先（<https://entreprenais.com/#contact>）を含め、ソースサイト運営者からの連絡を受け付けられるようにしています。
- **robots.txt を尊重**: ソースサイトの `robots.txt` を確認し、許可された範囲のみをクロールします。
- **リクエスト間に sleep**: 連続アクセスで負荷をかけないよう、リクエストの合間に待機（既定 5 秒）を挟む夜間バッチで実行します。
- **公開ページのみ対象**: 外部の飲食店情報サイト等の **公開ページのみ** を取得し、内部 API（`/API/` など）には一切アクセスしません。
- **問い合わせ先**: <https://entreprenais.com/#contact>
...
## 8. 公式エージェント（AI アンバサダー）

EntreprenAIs Factory のもう 1 本の柱が、公式ホームページ上で来訪者を案内する **公式エージェント（AI アンバサダー）** です。生成された静的ページに **JavaScript を組み込み、EntreprenAIs のエージェントシステムをページ上にオーバーレイ表示**することで、静的サイトの手軽さを保ったまま AI 対話を載せます。

### 8.1 現状（カテゴリ別）

| カテゴリ | 公式エージェントの状態 |
|---------|---------------------|
| **`cities`（自治体）** | **稼働中**。海士町（https://factory.entreprenais.com/cities/32525/）では AI アンバサダーが既に動作し、来訪者の質問に応答・案内します。 |
| **`foodre`（飲食店）** | **未実装**。今後、cities と同様の方式で各店舗ページにも公式エージェントを搭載する **想定**です。 |

### 8.2 設計

- **組み込み方式**: 静的 HTML に **JavaScript スニペットを 1 つ組み込む**だけで、ページ上に AI アンバサダーのオーバーレイ UI（コンタクト/チャット）が表示されます。ページ本体は静的のまま、対話部分のみをクライアントサイドの JS が担います。
- **同一アーキテクチャ**: この公式エージェントは、現在 **entreprenais.com に載っている AI（コンタクト/チャットエージェント）と同じアーキテクチャ**です。Factory 専用の独自実装ではなく、EntreprenAIs のエージェント基盤をそのまま各公式ホームページに展開する形になります。
- **知識ベース = SSOT**: 各エージェントは、対象の生成元データ（飲食店は `store.json`、自治体は `municipality.json`。いずれも SSOT）を知識ベースとして参照し、その店舗 / 自治体に関する案内を行えます。HTML と公式エージェントが同じ SSOT を源泉とするため、ページ表示と AI の案内内容が一貫します。

> **現在の目標とする組み込み例**: https://entreprenais.com/sites/amacho/ — 海士町。デザインと公式エージェント（AI アンバサダー）のオーバーレイ組み込みを含めた、現在の目標とする実例です。各公式ホームページへのエージェント組み込みは、このサイトの完成度を基準に展開していきます（デザイン面の位置づけは「10.1 デザイン方針 / 目標とする基本デザイン」も参照）。

> **静的サイトとの両立**: 公式エージェントはクライアントサイド JS によるオーバーレイであるため、「2. ホスティング方法」で説明した静的ファイル配信（S3・GitHub Pages 等）の手軽さを損ないません。生成済みページをそのまま置けば、エージェントの組み込み済みカテゴリではアンバサダーも同時に有効になります。

---

## 9. fork / clone / 組み込み手順

> ※ 本リポジトリは将来 **private 化を予定**しています（2026年8月以降を想定）。その際は「public を fork」する共有方法が使えなくなり、共有方法が変わります（詳細は「14. リポジトリ公開範囲と移行計画」参照）。
...
> **互換性に関する注意**: パターン B（`store.json` スキーマ流用）・パターン C（ビルド機構の再利用）は、いずれも継続的に改良されるビルドフレームワーク／スキーマに依存します。これらは **バージョン間で互換性が維持されない**（破壊的変更がありうる）ため、安定して使うなら **パターン A（生成済み静的 HTML の再利用）を推奨** します。詳細は「2.7 ビルドフレームワークの互換性に関する注意」を参照してください。
...
### 10.1 デザイン方針 / 目標とする基本デザイン

> **現状の注記**: 現在の生成テンプレートは **暫定品質** であり、今後デザインを改良していく予定です。本セクションは、その改良の指針となる「目標とする基本デザイン」を示すものです。

想定する基本デザインは、下記のサイトのようなものです。今後の生成テンプレートは、これらのサイトの完成度（情報設計・余白・写真の見せ方・多言語の扱いなど）を基準に改良していきます。カテゴリごとに目標とする実例を示します。

- **目標デザイン例（foodre / 飲食店）**: https://entreprenais.com/sites/banwaen-ikebukuro/
- **目標デザイン例（cities / 自治体）**: https://entreprenais.com/sites/amacho/ — 海士町。**デザインに加え、公式エージェント（AI アンバサダー）の組み込みも含めた現在の目標**を示す実例です（デザイン＋エージェントオーバーレイの参考。詳細は「8. 公式エージェント（AI アンバサダー）」参照）。
...
本システムは **ソースサイト（外部の飲食店情報サイト・自治体公開ページ等）の公開ページのクロール** に基づいて `store.json`（SSOT）を構築します。そのため、**`store.json` に含まれる情報（写真・説明文・メニュー・口コミ等）には、クロール元（ソースサイト）の著作物が含まれる場合があります。**

- **個別の著作権確認が必要です**: 生成されたホームページや `store.json` のデータを **実際に利用・公開・再配布する際は、各コンテンツ（写真・説明文・メニュー・口コミ等）の著作権・利用条件を個別に確認し、必要な許諾を得てください。** クロールにより取得できることと、それを自由に利用・再配布できることは別問題です。
- **利用者の責任で判断・遵守してください**: 本システムは公開ページのクロールに基づいて情報を集約しますが、**個々のコンテンツの著作権の判断・権利処理・遵守は利用者の責任** で行ってください。本システムは、生成物に含まれる第三者コンテンツについて利用許諾を保証するものではありません。
- **オーナー / オペレーター入力分も同様**: LINE・Desktop・Slack 経由でオーナーやオペレーターが追加した情報についても、第三者の著作物（他サイトからの転載写真・文章等）が含まれていないか、入力元で確認してください。

> 要するに、**「クロールできた = 自由に使える」ではありません。** 公開・商用利用・再配布の前に、各著作物の権利関係を必ずご確認ください（上記「11. ライセンス・利用範囲」の利用条件確定とあわせて個別にご相談ください）。
...
2. **fork して試しやすい**: パートナーが「9. fork / clone / 組み込み手順」に沿って気軽に fork し、自社サイトへの組み込みを試せます。

### 14.2 方針（将来 private へ移行予定）
