...
> **互換性に関する注意**: パターン B（`store.json` スキーマ流用）・パターン C（ビルド機構の再利用）は、いずれも継続的に改良されるビルドフレームワーク／スキーマに依存します。これらは **バージョン間で互換性が維持されない**（破壊的変更がありうる）ため、安定して使うなら **パターン A（生成済み静的 HTML の再利用）を推奨** します。詳細は「2.6 ビルドフレームワークの互換性に関する注意」を参照してください。
